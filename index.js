    ...
    for(let i = 0; i < games.length; i++ ){
        ...
        const display = `
            <div class= "games"> 
                ...
                <h1>The name of this game is ${games[i].name}</h1>
                <p>It's description says ${games[i].description}</p>
            </div>
        `;
...
const countUnfunded = GAMES_JSON.filter( (game) => {
        return game.pledged < game.goal;
});

// create a string that explains the number of unfunded games using the ternary operator
let totalStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${countUnfunded.length} ${countUnfunded.length > 1 ? "games remain" : " game remains"} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const text = document.createElement("p");
text.innerHTML = totalStr;
...
//let [a, b, c, d, e, f, g, h, i, j, k] = sortedGames;
